...
# ============================================================================
# Source: pkudat_mechess.txt (full text, 9,897 lines, 426K chars)
#         https://www.nevo.co.il/law_html/law01/265_001.htm
#
# COMPLETE law: 15 chapters, 311 articles — all parsed and summarized.
# Data lives in _ordinance_data.py to keep this file readable.
# Critical articles have rich structure (definitions, methods, additions);
# all others have chapter, Hebrew title, and English summary.
#
...
from lib._ordinance_data import ORDINANCE_ARTICLES, ORDINANCE_CHAPTERS

# The comprehensive flat-keyed dict: each article by its number ("1", "2", "130", "133א", etc.)
CUSTOMS_ORDINANCE_ARTICLES = ORDINANCE_ARTICLES

# Chapter metadata: 17 chapters with Hebrew/English titles and article ranges
CUSTOMS_ORDINANCE_CHAPTERS = ORDINANCE_CHAPTERS


# Helper function for law article access
def get_ordinance_article(article_id: str) -> dict:
    """Return a specific article from the Customs Ordinance.

    Args:
        article_id: Article number (e.g., '1', '24', '62', '130', '133', '168',
                     '207', '223א', '223ב'). Uses Hebrew letters for sub-articles.
    Returns:
...
def get_ordinance_chapter(chapter_num) -> dict:
    """Return chapter metadata from the Customs Ordinance.

    Args:
        chapter_num: Chapter number (1-15) or string ('13א').
    Returns:
        Dict with title_he, title_en, articles_range, or empty dict.
    """
    # Try both int and string keys since chapters use mixed types
    result = CUSTOMS_ORDINANCE_CHAPTERS.get(chapter_num, {})
    if not result:
        # Try opposite type
        try:
            alt = int(chapter_num) if isinstance(chapter_num, str) else str(chapter_num)
            result = CUSTOMS_ORDINANCE_CHAPTERS.get(alt, {})
        except (ValueError, TypeError):
            pass
    return result


def get_articles_by_chapter(chapter_num: int) -> dict:
    """Return all articles belonging to a specific chapter.

    Args:
        chapter_num: Chapter number (e.g., 8 for valuation, 11 for agents).
    Returns:
        Dict of {article_id: article_data} for all articles in that chapter.
    """
    return {
        k: v for k, v in CUSTOMS_ORDINANCE_ARTICLES.items()
        if v.get("ch") == chapter_num
    }
...
def get_valuation_methods() -> list:
    ...
    """
    art_130 = CUSTOMS_ORDINANCE_ARTICLES.get("130", {})
...
def format_legal_context_for_prompt(chapters: list = None, phase: int = None) -> str:
    ...
    # --- Customs Ordinance key articles (311 articles, 15 chapters) ---
    parts.append("=== CUSTOMS ORDINANCE — KEY ARTICLES (פקודת המכס [נוסח חדש]) ===")
    parts.append(f"Full law embedded: {len(CUSTOMS_ORDINANCE_ARTICLES)} articles across "
                 f"{len(CUSTOMS_ORDINANCE_CHAPTERS)} chapters.")
    parts.append("")
    ...
    art_133 = CUSTOMS_ORDINANCE_ARTICLES.get("133", {})
    if art_133:
        parts.append("TRANSACTION VALUE ADDITIONS (Section 133 — what gets added to price):")
        for add in art_133.get("additions", []):
            parts.append(f"  {add}")
        if art_133.get("key"):
            parts.append(f"  KEY: {art_133['key']}")
        parts.append("")

    # Key definitions (Section 1)
    parts.append("KEY DEFINITIONS (Section 1):")
    defs = CUSTOMS_ORDINANCE_ARTICLES.get("1", {}).get("definitions", {})
    for term, meaning in list(defs.items())[:10]:
        parts.append(f"  {term}: {meaning}")
    parts.append("")

    # Import declaration (Section 62) — daily broker work
    art_62 = CUSTOMS_ORDINANCE_ARTICLES.get("62", {})
    if art_62:
        parts.append(f"IMPORT DECLARATION (Section 62): {art_62.get('s', '')}")
    art_63 = CUSTOMS_ORDINANCE_ARTICLES.get("63", {})
    if art_63:
        parts.append(f"  Deadline (§63): {art_63.get('s', '')}")
    parts.append("")

    # Agent obligations
    art_168 = CUSTOMS_ORDINANCE_ARTICLES.get("168", {})
    art_169 = CUSTOMS_ORDINANCE_ARTICLES.get("169", {})
    parts.append("AGENT OBLIGATIONS (Sections 168-169):")
    if art_168:
        parts.append(f"  §168: {art_168.get('s', '')}")
    if art_169:
        parts.append(f"  §169: {art_169.get('s', '')}")
    parts.append("")

    # Drawback (Sections 155-156) — important for re-exports
    art_155 = CUSTOMS_ORDINANCE_ARTICLES.get("155", {})
    art_156 = CUSTOMS_ORDINANCE_ARTICLES.get("156", {})
    if art_155 or art_156:
        parts.append("DRAWBACK (Sections 155-156):")
        if art_155:
            parts.append(f"  §155: {art_155.get('s', '')}")
        if art_156:
            parts.append(f"  §156: {art_156.get('s', '')}")
        parts.append("")

    # Penalties summary
    parts.append("PENALTIES (Sections 207-223):")
    for art_id in ["211", "220", "217", "218"]:
        art = CUSTOMS_ORDINANCE_ARTICLES.get(art_id, {})
        if art:
            parts.append(f"  §{art_id} ({art.get('t', '')}): {art.get('s', '')}")
    ...
    for art_id in ["223ב", "223ד", "223יד", "223יח"]:
        art = CUSTOMS_ORDINANCE_ARTICLES.get(art_id, {})
        if art:
            parts.append(f"  §{art_id} ({art.get('t', '')}): {art.get('s', '')}")
    parts.append("")
