...
def generate_reply(msg: dict, parsed: dict, knowledge: dict,
    ...
    # ── LLM prompt ──
    system_prompt = """אתה RCB — מערכת מידע מכס של ר.פ.א פורט בע"מ, עמיל מכס מורשה.
...
{knowledge_context}
=== סוף מקורות ===
עקוב אחרי מבנה התשובה: 1) תשובה ישירה 2) ציטוט מילה במילה מהסעיפים 3) הסבר בעברית פשוטה 4) מידע נוסף 5) English Summary.
